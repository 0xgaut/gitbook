# Resources

Access support, brand assets, and community resources for Universal Protocol.

## Quick Links

<table>
  <tr>
    <td><strong><a href="support.md">Support & FAQ</a></strong><br/>Get help with Universal Protocol and find answers to common questions</td>
  </tr>
  <tr>
    <td><strong><a href="brand-kit.md">Brand Kit</a></strong><br/>Download logos, colors, and brand guidelines</td>
  </tr>
</table>

## Community

Join the Universal community to stay updated on protocol developments and get support:

- **[Discord](http://discord.gg/universalassets)** - Developer support and community discussions
- **[Twitter/X](https://twitter.com/universaldotxyz)** - Protocol updates and announcements

## Official Links

### Applications

- **[Universal App](https://app.universal.xyz)** - Trade Universal assets
- **[Mint & Redeem](https://www.universal.xyz/mint-and-redeem)** - Direct minting/redemption portal
- **[Proof of Reserves](https://www.universal.xyz/reserves)** - Verify reserve backing

### Documentation

- **[Whitepaper](https://app.universal.xyz/docs/universal-whitepaper.pdf)** - Technical whitepaper
- **[EVM Audit](https://github.com/r0bert-ethack/audits/blob/main/Alongside%20-%20Universal%20Contracts%20report%20-%20Final.pdf)** - Smart contract audit
- **[Solana Audit](https://hacken.io/audits/universal/)** - Solana program audit

### Legal

- **[Disclaimer](https://app.universal.xyz/disclaimer)** - Legal disclaimer
- **[Terms of Service](https://app.universal.xyz/term-of-service)** - Terms and conditions

## Support Channels

### For Users

- **[Support Documentation](support.md)** - FAQs and troubleshooting
- **[Discord Community](http://discord.gg/universalassets)** - Community support
- **Email**: support@universal.xyz

### For Developers

- **[Developer Documentation](../developers/)** - Technical integration guides
- **[Discord Developer Chat](http://discord.gg/universalassets)** - Technical support
- **Email**: dev@universal.xyz

### For Partnerships

- **[Core Contributors](../introduction/core-contributors.md)** - Partnership contacts
- **Email**: austin@universal.xyz

## Additional Resources

### Learn

- [Introduction to Universal](../introduction/what-is-universal.md)
- [How Trading Works](../trade/how-it-works.md)
- [Protocol Overview](../developers/protocol.md)

### Build

- [Universal API](../developers/api.md)
- [TypeScript SDK](../developers/sdk.md)
- [Smart Contracts](../developers/smart-contracts.md)

### Trade

- [Placing a Trade](../trade/placing-a-trade.md)
- [Referral Program](../trade/referrals.md)
- [Trading Risks](../trade/risks.md)

## Stay Updated

Follow Universal for the latest updates:

- **Twitter/X**: [@universaldotxyz](https://twitter.com/universaldotxyz)
- **Discord**: [discord.gg/universalassets](http://discord.gg/universalassets)
- **Website**: [universal.xyz](https://www.universal.xyz)
